# Checklist pré-deploy (ASVS nível L1)

Os 70 requisitos de nível L1 do OWASP ASVS 5.0.0 — o piso mínimo
inegociável antes de qualquer deploy em produção (ver
[`owasp-asvs`](../skills/owasp-asvs/SKILL.md) para os níveis L2/L3 e o
padrão completo). Extraído mecanicamente de
`skills/owasp-asvs/references/V*.md`; se a skill for atualizada via
`scripts/sync-owasp-asvs.py`, regenere esta lista.

Use antes de colocar um SaaS novo em produção, ou como critério de
aceite de uma feature que toca autenticação, sessão, dado sensível ou
configuração.
